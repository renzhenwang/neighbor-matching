...
def main():
    ...
    if args.resume:
        ...
        logger.set_names(
            ['Train Loss', 'Train Loss X', 'Train Loss U', 'Valid Loss', 'Valid AUC', 'Test Loss', 'Test AUC', 'Test ACC', 'Test MCR'])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # append logger file
        logger.append([train_loss, train_loss_x, train_loss_u, val_loss, val_auc, test_loss, test_auc, test_acc, test_mcr])
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, criterion, epoch, use_cuda, mixup='input', noSharp=False, alr=None):
    ...
    for batch_idx in range(args.val_iteration):
        ...
        with torch.no_grad():
            ...
            outputs_u = model(inputs_u, is_matching=True)
            outputs_u2 = model(inputs_u2, is_matching=True)
